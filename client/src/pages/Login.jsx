...
function Login() {
  ...
  const login = async () => {
    try {
      ...
      console.error(error);
      setErrorMessage(error.response?.data?.message || error.response?.data?.error || "Login failed. Please try again.");
    }
  };
      ...
      <div className="mx-auto grid min-h-[calc(100vh-4rem)] w-full max-w-6xl items-center gap-8 lg:grid-cols-2">
        ...
        <section className="mx-auto w-full max-w-md rounded-2xl border border-white/70 bg-white p-6 shadow-xl sm:p-8">
          <div className="mb-8 text-center">
            ...
            <h2 className="text-3xl font-bold tracking-tight text-gray-950">Welcome Back</h2>
            <p className="mt-2 text-sm text-gray-500">Login to continue managing your expenses</p>
          </div>
          ...
          <p className="mt-6 text-center text-sm text-gray-500">
            ...
            <Link to="/register" className="font-semibold text-sky-700 transition hover:text-emerald-600">
              Create Account
            </Link>
          </p>
        </section>
